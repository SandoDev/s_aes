...
from s_aes.logic.methods import add_key, nibble_substitution, shift_row, mix_column_inverse
...
def round_1(matrix_bin, key):
    new_state1 = shift_row(matrix_bin, inverse=True)
    new_state2 = nibble_substitution(new_state1, inverse=True)
    ...
    new_state3 = matrixbin_to_hex(new_state3)

    new_state4 = mix_column_inverse(new_state3)
...
def round_2(matrix_bin, key):
    new_state1 = shift_row(matrix_bin, inverse=True)
    new_state1 = matrixhex_to_bin(new_state1)
    new_state2 = nibble_substitution(new_state1, inverse=True)
...
def decrypt_saes(value_to_decrypt, keys):
    k0, k1, k2 = keys

    list_in_hex, _ = ascii_to_hex(value_to_decrypt, step=1)
    ...
    # Round 0
    new_state = round_0(matrix_in_bin, k2)
